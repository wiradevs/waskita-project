...
@section('content')
<div class="max-w-lg">
    <form method="POST" action="{{ route('panel.categories.update', $category) }}" enctype="multipart/form-data"
        ...
        <div class="card p-6 space-y-5">
            ...
            <x-admin.textarea name="description" label="Deskripsi" :value="$category->description" rows="3" />
            <x-admin.file-upload name="image" label="Gambar Kategori" accept="image/*"
                                  :current="$category->image" type="image" />

            <label class="flex items-center justify-between cursor-pointer py-1">
                <span class="text-sm text-stone-600">Kategori Aktif</span>
                <div x-data="{ on: {{ $category->is_active ? 'true' : 'false' }} }">
            ...
            <x-admin.input name="sort_order" label="Urutan tampil" :value="$category->sort_order" type="number" />
        </div>
